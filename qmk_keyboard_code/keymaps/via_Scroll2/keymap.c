...
void matrix_scan_user(void) {
  low_cpi_mode = isScrollMode || isPointM_low;
  if (low_cpi_mode != last_low_cpi_mode) {
    last_low_cpi_mode = low_cpi_mode;
    if (low_cpi_mode) {
      wait_ms(1);
      pointing_device_set_cpi(125);
      wait_ms(1);
    } else {
      wait_ms(1);
      pointing_device_set_cpi(975);
...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    if (isScrollMode==true) {
        temp_x = (int16_t)(-((float)mouse_report.x - (float)mouse_report.y));
        temp_y = (int16_t)(((float)mouse_report.x + (float)mouse_report.y));
        // temp_x = (int16_t)(mouse_report.x - mouse_report.y);
        // temp_y = (int16_t)(mouse_report.x + mouse_report.y);
        mouse_report.h = temp_x;
        mouse_report.v = temp_y;
        mouse_report.x=0;
        mouse_report.y=0;
    }
    else if(isPointM_low==true){
        mouse_report.h = 0;
        mouse_report.v = 0;
        temp_x = (int16_t)(-((float)mouse_report.x - (float)mouse_report.y));
        temp_y = (int16_t)(-((float)mouse_report.x + (float)mouse_report.y));
